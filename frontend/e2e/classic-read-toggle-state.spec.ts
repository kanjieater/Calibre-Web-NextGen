...
test('Classic catalog and detail checkboxes stay state-consistent before and after a toggle (#771)', async ({ page, isMobile }) => {
  ...
  const card = page.locator(`a.book-cover-link[data-book-id="${bookId}"]`);
  ...
  await expect(card.locator('.cover-badge-read')).toHaveCount(initiallyRead ? 1 : 0);
  ...
  try {
    ...
    await expect(await toggleResponse).toBeOK();
    restored = false;
    ...
    await expect(card.locator('.cover-badge-read')).toHaveCount(initiallyRead ? 0 : 1);
    ...
    await expect(await restoreResponse).toBeOK();
